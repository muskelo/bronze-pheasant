...
func (sm *SyncManager) run(ctx context.Context) error {
	...
	for _, file := range files {
		...
		if err != nil {
            log.Errorf("Failed to get the list of nodes within file %v: %v\n. Skip...\n", file.UUID, err)
            continue
		}
        ...
        for _, node := range nodes {
            ...
            if err != nil {
                log.Errorf("Failed get file %v from node %v: %v\n", file.UUID, node.Name, err)
            } else {
                resp = tmpResp
                break
            }
        ...
        if resp == nil {
            log.Errorf("Failed to download file %v from any node. Skip...", file.UUID)
            continue
        }

		_, err = sm.storage.WriteFile(file.UUID, resp.Body)
		if err != nil {
            log.Errorf("Failed to write file %v on disk: %v. Skip...\n", file.UUID, err)
            continue
		}
